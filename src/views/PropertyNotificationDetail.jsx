...
const PropertyNotificationDetail = () => {
  const { id } = useParams();
  const { user } = useAuth();
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ['property-notification', id],
    queryFn: () => notificationsAPI.myGetById(id).then((res) => res.data.notification),
    enabled: !!id && !passedNotification,
    initialData: passedNotification || undefined,
  });

  const markReadMutation = useMutation({
    mutationFn: (notificationId) => notificationsAPI.myMarkAsRead(notificationId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['property-notification', id] });
      queryClient.invalidateQueries({ queryKey: ['property-notifications', user?.email] });
    },
  });

  const deleteMutation = useMutation({
    mutationFn: (notificationId) => notificationsAPI.myDelete(notificationId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['property-notifications', user?.email] });
      navigate('/property-notifications', { replace: true });
    },
  });

  // Fetch property details when notification is for a new property
  ...
  if (isError) {
    return (
      <div className="p-10 max-w-7xl mx-auto">
        ...
        >
          Back to Property Notifications
        </button>
      </div>
    );
  }
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  ...
                  {property.expectedCloseDate && (
                    <div className="bg-app rounded-lg p-4">
                      ...
                      <p className="text-sm text-text-primary font-medium">
                        {new Date(property.expectedCloseDate).toLocaleDateString('en-US', {
...
export default PropertyNotificationDetail;
