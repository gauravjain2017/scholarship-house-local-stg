...
const FileUpload = ({
  ...
  const makePreview = (item) => {
    if (typeof item === 'string') {
      const type = isVideoUrl(item) ? 'video' : isPdfUrl(item) ? 'pdf' : 'image';
      return { url: item, type, name: item.split('/').pop(), file: null };
    }
    ...
    if (item instanceof File) {
      const type = item.type.startsWith('video')
        ...
        ? 'pdf'
        : 'image';
      return { url: URL.createObjectURL(item), type, name: item.name, file: item };
    }
          ...
          <div className="text-center">
            ...
            <p className="text-xs text-gray-500 mt-1">
              {accept === 'image/*'
                ...
                : accept === 'video/*'
                  ? 'Videos only'
                  : accept?.includes('application/pdf') && accept?.includes('image/')
                    ? 'Images and PDFs'
              ...
              >
                {item.type === 'pdf' ? (
                  <a
                    ...
                    className="flex flex-col items-center justify-center w-full h-24 rounded border border-gray-200 bg-gray-50 hover:bg-gray-100 transition-colors gap-1"
                    style={{ pointerEvents: 'auto' }}
                    onClick={(e) => e.stopPropagation()}
                  >
                    <svg className="w-10 h-10 text-red-400" fill="currentColor" viewBox="0 0 24 24">
                      <path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8l-6-6zm-1 1.5L18.5 9H13V3.5zM6 20V4h5v7h7v9H6z" />
                      <text x="7" y="17" fontSize="4" fontWeight="bold" fill="#ef4444">PDF</text>
                    </svg>
                  </a>
                ) : item.type === 'image' ? (
                  <img
