...
// Card carousel: cover photo + exterior only — keeps listing page fast
...
const DealCard = ({
  ...
  return (
    <div
      className={`group bg-surface border border-border-subtle rounded-2xl overflow-hidden shadow-sm transition hover:shadow-lg hover:-translate-y-1 h-full flex flex-col${!showFilterSidebar ? ' scroll_filter' : ''} ${showFilterSidebar ? 'sidebarShown' : 'SidebarNotShow'}`}
    >
      {/* ── Image ── */}
      <div className="relative h-48 bg-app flex-shrink-0">
        ...
        {/* SOLD badge — shown only when deal status is SOLD */}
        ...
        {/* PENDING badge — shown only when deal status is PENDING */}
        {deal.status?.toUpperCase() === 'PENDING' && (
          <>
            {/* Dim overlay */}
            <div className="absolute inset-0 z-10 bg-orange-900/20 rounded-t-2xl pointer-events-none" />
            {/* Badge */}
            <div className="absolute top-3 left-3 z-30 flex items-center gap-1 bg-orange-500 text-white text-[11px] font-extrabold uppercase tracking-widest px-3 py-1.5 rounded-full shadow-lg ring-2 ring-white">
              <svg xmlns="http://www.w3.org/2000/svg" className="w-3 h-3" viewBox="0 0 20 20" fill="currentColor">
        ...
        {/* ACTIVE badge — shown only when deal status is ACTIVE */}
        {deal.status?.toUpperCase() === 'ACTIVE' && (
          <div className="absolute top-3 left-3 z-30 flex items-center gap-1 bg-emerald-500 text-white text-[11px] font-extrabold uppercase tracking-widest px-3 py-1.5 rounded-full shadow-lg ring-2 ring-white">
      ...
      <div className="p-4 flex flex-col flex-1 gap-3">
          ...
          <div className="rounded-xl border border-gray-100 bg-gray-50">
            ...
            <div className="p-2 overflow-y-auto scroll_bar">
              <table className="w-full text-xs table_width tier-table">
                ...
                <tbody>
                  {tierRows.map((row) => (
                    <tr key={row.tier} className="border-t border-gray-200 tier-center">
        ...
        <div className="flex items-center justify-between mt-auto pt-1">
          {(deal.status?.toUpperCase() !== 'PENDING' && useHasPermission('browse_property.copied_link')) ? (
            <button
