...
// Only these roles are permitted to log in to the admin portal.
...
export const AuthProvider = ({ children }) => {
      ...
      try {
        ...
        if (!token) {
          ...
          if (stored) {
            ...
            // Defensive: if the stored user is not an allowed admin role
            // (e.g. a client session left over in the same browser), do
            // NOT restore it. Clear it so nothing leaks into the admin app.
            // Skip this check if roles haven't loaded yet to avoid a race condition.
        ...
        // in the admin portal. Tear down the session immediately.
        // Skip this check if roles haven't loaded yet to avoid a race condition.
        if (ALLOWED_ROLES.length > 0 && !ALLOWED_ROLES.includes(normalized.user.role)) {
          logout();
          return;
  ...
  // login. In this admin interface it defaults to 'admin', which on the
  // backend allows only roles "admin" and "team_member" through. We ALSO
  // re-verify the role on the frontend before creating any session state,
  // so a user with any other role is never signed in here.
  const submitterLogin = async (email, password, type = 'admin') => {
    try {
      const response = await authAPI.submitterLogin(email, password, type);
      const normalized = normalizeAuthUser(response);
      // console.log('Fetch Role in : ',roles)
      // console.log('response : ',response);
      // console.log('normalized : ',normalized);
      // return;

      // Defensive frontend guard: only admin / team_member are allowed here.
      ...
      // is created for a non-admin / non-team_member user.
      if (ALLOWED_ROLES.length > 0 && !ALLOWED_ROLES.includes(normalized.user.role)) {
        return {
          success: false,
          error:
            'Access denied. Your account role is not authorized to sign in to this portal.',
          code: 'INVALID_ROLE',
        };
      }
