...
const PropertyManagement = () => {
  ...
  useEffect(() => {
    ...
    for (let num = 1; num <= 10; num++) {
      ...
      const computed = Math.round((occ / 100) * rate);
  ...
  // Auto-compute AGR (Assumed Gross Revenue) and derived expenses
  // AGR        = 365 × (Occupancy Rate % / 100) × Average Night Rate
  // Sales Tax  = AGR × 6%
  // OTA Fees   = AGR × 15%
  // Management = AGR × 25%
  useEffect(() => {
    if (!underwritingDeal) return;
    const occ = Number(underwritingDeal.occupancyRate) || 0;
    const nightRate = Number(underwritingDeal.averageNightRate) || 0;
  ...
  }, [underwritingDeal?.occupancyRate, underwritingDeal?.averageNightRate]);
  ...
  // Principal & Interest = Total Monthly Payment (pass-through)
  useEffect(() => {
    if (!underwritingDeal) return;
    ...
    setUnderwritingDeal((p) => ({
      ...p,
      expensePrincipalInterest: monthlyPayment ? String(monthlyPayment) : '',
    }));
  ...
  const openUnderwriting = (deal) => {
    ...
    seedIfEmpty('expensePrincipalInterest', Math.round(monthlyPayment));
              ...
              <section className="space-y-6">
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-blue-50 border border-blue-100 rounded-lg px-4 py-3">
                    <div className="text-xs text-text-secondary font-medium mb-1">AGR (Assumed Gross Revenue) — 365 × Occupancy × Average Night Rate</div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <NumericInput
                    ...
                    onChange={(e) => {
                      ...
                      incomeReductionManualRef.current = next !== '';
