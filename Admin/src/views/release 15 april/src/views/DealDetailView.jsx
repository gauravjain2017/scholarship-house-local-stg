...
const LLC_INCLUDED_ITEMS = [
  { icon: '🏠', category: 'Acquisition', items: ['Earnest Money Deposit', 'Home Inspection', 'Title Report', 'Down Payment', 'Entry Fee', 'Closing Costs', 'Real Estate Agent Commissions', 'Wholesale Fees'] },
      ...
      <div className="absolute bottom-5 left-1/2 flex -translate-x-1/2 gap-2 overflow-x-auto rounded-full bg-white/10 px-3 py-2 backdrop-blur">
        {images.map((img, index) => (
          <img
            ...
            className={`h-14 w-14 cursor-pointer rounded-lg object-cover ring-2 ${
              index === current ? 'ring-white' : 'ring-transparent opacity-70'
            }`}
          />
        ))}
      </div>
...
const DetailRow = ({ label, value, emphasized = false }) => (
  <div
    className={`flex items-center justify-between gap-4 rounded-2xl md:px-4 px-1 py-2 ${
      emphasized ? 'bg-white shadow-sm ring-1 ring-sky-100 total_row' : ''
    }`}
  >
        ...
        <thead>
          <tr className="border-b border-slate-200 text-left">
            {columns.map((column) => (
              <th
                key={column.key}
                className={`pb-3 text-xs font-semibold uppercase tracking-[0.14em] text-gray-600 ${
          ...
          <div className="space-y-5">
            <div>
              ...
              <SoftCard className="mt-2">
                ...
                <StatItem label="Down Payment" value={fmt$(deal.downPayment)} />
				<StatItem
...
<section className="mt-12 border-t border-slate-200 pt-10">
  ...
  <SoftCard className="p-5 border border-gray-100 bg-white rounded-xl">
    <div className="mb-5 flex items-start gap-3 rounded-xl border-l-4 border-amber-400 bg-gradient-to-r from-amber-50 to-transparent px-4 py-3">
...
{(() => {
  ...
  const annualFields = [
    ...
    { label: 'Utilities and Expenses', value: deal.expenseUtilities },
