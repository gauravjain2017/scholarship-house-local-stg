...
const UserPlusIcon = () => (
  ...
  >
    <path
      ...
      strokeWidth={2}
      d="M18 9v6m3-3h-6M13 7a4 4 0 11-8 0 4 4 0 018 0zm-4 7c-3.314 0-6 2.686-6 6h12c0-3.314-2.686-6-6-6z"
    />
  </svg>
);

const canSubmit = (role, allRoles = []) => {
  const submitterSlugs = allRoles.filter((r) => r.portal_type === 'submitter').map((r) => r.role_slug);
  const adminSlugs = allRoles.filter((r) => r.portal_type === 'admin').map((r) => r.role_slug);
  return adminSlugs.includes(role) || submitterSlugs.includes(role);
};

const canBrowse = (role, allRoles = []) => {
  const clientSlugs = allRoles.filter((r) => r.portal_type === 'client').map((r) => r.role_slug);
  const adminSlugs = allRoles.filter((r) => r.portal_type === 'admin').map((r) => r.role_slug);
  return adminSlugs.includes(role) || clientSlugs.includes(role);
};

const canAccessAdmin = (role, allRoles = []) =>
  allRoles.filter((r) => r.portal_type === 'admin').map((r) => r.role_slug).includes(role);
...
const getRoleLabel = (role, allRoles = []) => {
  ...
  if (dbRole) return `Scholarship House ${dbRole.role_name}`;
  const fallback = {
    admin: 'Administrator',
    submitter: 'Submitter',
    client: 'Client',
...
const Home = () => {
  ...
  const visiblePendingUsers = user?.userType === 'team_member'
    ? pendingUsers.filter(u => u?.specialist === user?.email)
    : pendingUsers;
  const pendingCount = visiblePendingUsers.length;
  ...
  const showBrowse = canBrowse(role, allRoles);
  const isAdminLike = canAccessAdmin(role, allRoles);
  const showPieChart = isAdminLike || PIE_CHART_EMAILS.includes(user?.email?.toLowerCase());
  const isSubmitterOnly = canSubmit(role, allRoles) && !isAdminLike;

  return (
    <div className="bg-app min-h-screen">

      <div className="md:p-10 p-7 max-w-7xl mx-auto">
        <div className="bg-surface border border-border-subtle rounded-3xl md:p-10 p-6 md:mb-5 mb-6 shadow-sm welcome-box">
		
          <h1 className="text-2xl md:text-4xl font-bold text-primary mb-3">
            Welcome{getDisplayName(user) ? `, ${getDisplayName(user)}` : ''}
          </h1>
          <p className="text-base text-text-secondary">
            You are {withIndefiniteArticle(getRoleLabel(role, allRoles))}.
          </p>
          <div className="md:mt-3 mt-1 h-1 w-32 bg-accent rounded-full divider_color" />
        </div>
        {/* ---------- How this platform works ---------- */}
        <div className="bg-surface border border-border-subtle rounded-3xl md:p-10 md:mb-8 shadow-sm p-6 mb-5">
          <h2 className="text-xl font-bold text-primary mb-3">How this platform works</h2>
        ...
        {(useHasPermission('user_management.user_listing') || useHasPermission('user_management.own_user_listing')) && <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-7">
          {[
            { label: 'Pending Registrations', value: pendingCount, sub: `${pendingCount} awaiting approval`, colorClass: 'text-blue-500',iconClass:'pending_icon' ,borderClass: 'bg-blue-500 dark-blue', loading: pendingLoading, path: '/admin/users?type=pending', state: { from: 'dashboard' } },
            { label: 'Active Users', value: activeUsersCount, sub: `${activeUsersCount} users active`, colorClass: 'text-teal-500',iconClass:'active_icon', borderClass: 'bg-teal-500 green-border', loading: usersLoading, path: '/admin/users?type=active' },
            { label: 'Deactive User', value: inActiveUserCount, sub: inActiveUserCount > 0 ? `${inActiveUserCount} users inactive` : 'No inactive users', colorClass: 'text-red-500', borderClass: 'bg-red-500 red-border', loading: usersLoading,iconClass:'deactivate_icon', path: '/admin/deactivated-users' },
            { label: 'Total Users', value: totalUsers,iconClass:'final_icon', sub: 'Total registered accounts', colorClass: 'text-green-500', borderClass: 'bg-green-500', loading: usersLoading, path: '/admin/users' },
          ].map(({ label, value, sub, colorClass, borderClass, loading, path, state,iconClass }) => (
            <div key={label} onClick={() => navigate(path, { state })} className="bg-surface border border-border-subtle rounded-2xl shadow-sm overflow-hidden cursor-pointer hover:shadow-md transition-shadow">
              <div className={`h-1 w-full ${borderClass}`} />
              <div className="p-6">
			  <div className={`w-10 h-10 rounded-xl bg-blue-100 flex items-center justify-center text-blue-500 ${iconClass}`}>
			  <UserPlusIcon />
			</div>
                <p className="text-xs font-bold uppercase tracking-widest text-text-secondary mb-3">{label}</p>
                {loading ? (
                  <>
        ...
        {showPieChart && <div className="bg-surface border border-border-subtle rounded-2xl shadow-sm mb-7 overflow-hidden">
          <div className="flex items-center justify-between px-6 py-5 border-b border-border-subtle">
            <h2 className="text-base font-bold text-primary">Property Status Overview</h2>
          </div>
          <div className="p-6">
            {dealsLoading ? (
        ...
        {useHasPermission('user_management.user_listing') && <div className="bg-surface border border-border-subtle rounded-2xl shadow-sm mb-10 overflow-hidden">
          <div className="flex items-center justify-between px-6 py-5 border-b border-border-subtle">
            <h2 className="text-base font-bold text-primary">Pending User Registrations</h2>
            <Link to="/admin/users?type=pending" state={{ from: 'dashboard' }} className="text-sm text-blue-500 hover:underline view-all-btn">View all →</Link>
          </div>
          <table className="w-full text-sm">
            <thead className="border_bottom_color">
              <tr className="text-xs font-semibold uppercase tracking-widest text-text-secondary">
                ...
                {(useHasPermission('user_management.approve_user') || useHasPermission('user_management.deactivate')) && <th className="text-left px-6 py-3">Actions</th>}
              </tr>
            </thead>
                      ...
                      <tr key={u.email} className="hover:bg-panel transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            {useHasPermission('user_management.approve_user') && <button
                            ...
                            {useHasPermission('user_management.reject_user') && <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-surface rounded-2xl shadow-xl w-full max-w-md mx-4 p-8">
            ...
            <div className="bg-panel border border-border-subtle rounded-xl p-4 mb-7 space-y-1 text-sm">
              ...
              <p><span className="font-semibold text-primary">Requested Role:</span> <span className="text-text-secondary">{getRoleLabel(actionTarget.user?.requestedUserType ?? normalizeRole(actionTarget.user), allRoles)}</span></p>
            </div>
