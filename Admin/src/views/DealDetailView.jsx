...
export const getDealImages = (deal) =>
  [
    ...(Array.isArray(deal?.coverPhoto) ? deal.coverPhoto : deal?.coverPhoto ? [deal.coverPhoto] : []),
...
const DealDetailView = ({
  ...
  console.log('Rendering DealDetailView for deal:', deal);
        ...
        <div className="p-6 px-3 md:px-8 md:p-8 rounded-2xl border border-slate-200 bg-slate-50/90">
	      ...
	      {hasValue(deal.story) && (
            <section className="mt-12 border-t border-slate-200 pt-10">
              <SectionHeading title="Story" icon="✦" />
              <p className="text-sm text-gray-600">{deal.story}</p>
            </section>
          )}
          ...
          {Array.isArray(deal.underwritingImages) && deal.underwritingImages.length > 0 && (
            <section className="mt-10 border-t border-slate-200 pt-10">
              ...
              {(() => {
                ...
                const imageUrls = uwImages.filter((u) => !/\.pdf$/i.test(u));
                ...
                return (
                  <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 p-6 bg-white rounded-xl border border-gray-100">
                    {visibleImages.map((url, idx) => {
                      ...
                      const isLast = !isPdf && idx === MAX_VISIBLE - 1 && remaining > 0;
                      ...
                      if (isPdf) {
                        return (
                          <a
                            ...
                            target="_blank"
                            rel="noopener noreferrer"
                            className="group flex flex-col items-center justify-center gap-2 h-40 rounded-xl border border-gray-200 bg-gray-50 hover:bg-red-50 hover:border-red-200 transition-colors px-3"
                          >
                            <svg className="w-12 h-12 text-red-400 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
                              <path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8l-6-6zm-1 2l5 5h-5V4zM6 20V4h5v7h7v9H6z" />
                            </svg>
                            <span className="text-xs text-center text-gray-600 group-hover:text-red-700 font-medium line-clamp-2 break-all leading-snug">
                              {decodeURIComponent(fileName)}
                            </span>
                            <span className="text-[10px] text-gray-400 group-hover:text-red-400 uppercase tracking-wide">PDF — click to open</span>
                          </a>
                        );
                      }
                      ...
                      return (
                        <button
                          ...
                          onClick={() => setLightbox({ images: imageUrls, index: imageUrls.indexOf(url) })}
                        >
                          <img
                            src={url}
          ...
          {/* Property PDF — rendered inline as a stack of canvas pages
              (via PdfPagesViewer) so the document reads like part of
              the HTML document: no PDF-viewer chrome, no toolbar, no
              inner scroll. The page itself scrolls. Open / Download
              links above the frame let the user grab the file. */}
...
const DealDetailPage = () => {
  ...
  const handleBack = () => {
    ...
    else if (location.state?.from === '/claim-properties') navigate('/claim-properties');
    else navigate('/deals');
  };
  ...
  const backLabel =
    location.state?.from === 'admin-properties'
      ...
      : location.state?.from === '/favorite-properties'
        ...
        : location.state?.from === '/admin/properties'
          ? 'Back to Properties'
          : location.state?.from === '/claim-properties'
            ? 'Back to Claim Properties'
            : 'Back to Deals';
