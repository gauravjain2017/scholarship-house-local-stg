...
const PropertyNotification = () => {
  ...
  // Uses /api/notifications/my — no admin role required
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ['property-notifications', user?.email],
    queryFn: () => notificationsAPI.getMyNotifications().then((res) => res.data.notifications),
    enabled: !!user?.email,
  });

  const markReadMutation = useMutation({
    mutationFn: (id) => notificationsAPI.myMarkAsRead(id),
    onSuccess: () =>
      queryClient.invalidateQueries({ queryKey: ['property-notifications', user?.email] }),
  });

  const deleteMutation = useMutation({
    mutationFn: (id) => notificationsAPI.myDelete(id),
    onSuccess: () => {
  ...
  const handleNotificationClick = (n) => {
    navigate(`/property-notifications/${n.id}`);
  };
  ...
  return (
    <div className="p-10 max-w-7xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-semibold text-primary">Property Notifications</h1>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p
                        className={`text-sm ${
                          !n.notify ? 'font-semibold text-primary' : 'text-text-primary'
                        }`}
                      >
