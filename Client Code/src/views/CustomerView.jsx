...
const CustomerView = () => {
  ...
  const filteredDeals = (deals || []).filter((deal) => {
    ...
    if (filters.selectedTags?.length) {
      ...
      const matchesAny = filters.selectedTags.some((t) => tagMatchers[t]?.());
  ...
  const renderPropertyGrid = (gridCols = 'grid-cols-1 md:grid-cols-3 lg:grid-cols-2') => (
    <>
      {isLoading ? (
        <Loader />
  ...
  const forSaveFilters = () => {
    ...
    const excludeKeys = ['search', 'sortBy', 'propertyType', 'city', 'stateRegion', 'postalCode', 'downPayment', 'isHOA', 'hoaMonthlyFeeMax', 'hoaMonthlyFeeMin', 'strZoning', 'strConfidence', 'turnkey', 'submitterSearch', 'status', 'advShortTerm'];
    ...
    return (
      <div className="bg-app min-h-screen">
        ...
        <div className="flex">
          ...
          <div
            className={`flex-1 container mx-auto px-4 pt-6 pb-8 ${showFilterSidebar ? 'sidebar_visible' : 'sidebar_hide'
              }`}
          >
            {renderPropertyGrid(
              isAdmin
