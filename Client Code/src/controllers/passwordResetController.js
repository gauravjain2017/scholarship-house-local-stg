 ...
 */
exports.requestPasswordResetold = async (req, res) => {
  try {
   ...
   console.log(userType);
    ...
    // Custom-scheme deep link so users with the mobile app installed can open
    // the reset screen directly in the app. Scheme must match app.config.ts.
    const appScheme = process.env.MOBILE_APP_SCHEME || 'propsub-dev';
    const appDeepLink = `${appScheme}://reset-password?token=${encodeURIComponent(token)}`;

    // Send email
    try {
      await sendPasswordResetEmail(lowerEmail, resetUrl, 'user', user?.Name, userRole, appDeepLink);
...
exports.requestPasswordReset = async (req, res) => {
  try {
    ...
    // --- Fetch roles dynamically from DynamoDB (same as login) ---
    ...
    const requestedType = String(userType || '').toLowerCase();
    ...
    console.log('typeToAllowedRoles : ',typeToAllowedRoles)
    ...
    // Verify the found user's role matches the allowed roles for the requested portal type
    const foundType = (user?.UserType || user?.role || '').toLowerCase();

    if (!allowedRoles.includes(foundType)) {
      console.log(
        `Type mismatch for ${lowerEmail}: requested ${requestedType}, found ${foundType}`
      );
      return res.json(errorResponse);
    }
...
/**
 * Request password reset — email-only variant.
 * POST /api/password/request-reset-by-email
 *
 * Differences from `requestPasswordReset` above (kept untouched for the
 * existing client + submitter websites):
 *   - Caller does NOT send `userType`. We look the email up and infer the
 *     stored UserType from the record, so a submitter email gets the
 *     submitter portal link and a client email gets the client portal link.
 *   - Unknown emails return HTTP 404 with a clear error so the mobile
 *     forgot-password screen can surface "No account found." This trades
 *     email-enumeration protection for a friendlier UX, per product request.
 */
exports.requestPasswordResetByEmail = async (req, res) => {
  try {
    ...
    const lowerEmail = email.toLowerCase();
    const user = await Submitter.findByEmail(lowerEmail);

    if (!user) {
      console.log(`Password reset (by email) requested for non-existent email: ${lowerEmail}`);
    ...
    const inferredType = String(user.UserType || user.role || 'submitter').toLowerCase();
    const userRole = inferredType
      .replace(/_/g, ' ')
    ...
    const token = generateToken();
    await storeResetToken(token, lowerEmail, 'user', inferredType);

    const frontendUrl = getFrontendUrlByUserType(inferredType);
    const resetUrl = `${frontendUrl}/auth/reset-password/${token}`;

    // Custom-scheme deep link so users with the mobile app installed can
    // open the reset screen directly inside the app. The scheme matches the
    // value in `app.config.ts` — `propsub-dev` (development),
    // `propsub-staging`, or `propsub` (production). Set MOBILE_APP_SCHEME
    // to switch environments without redeploying.
    const appScheme = process.env.MOBILE_APP_SCHEME || 'propsub-dev';
    const appDeepLink = `${appScheme}://reset-password?token=${encodeURIComponent(token)}`;
    ...
    try {
      ...
      console.log(`Password reset email sent to: ${lowerEmail} (type: ${inferredType})`);
    } catch (emailError) {
      console.error('Failed to send password reset email:', emailError);
    }
...
exports.adminTriggerPasswordReset = async (req, res) => {
  try {
    ...
    // Send email
    try {
      await sendPasswordResetEmail(lowerEmail, resetUrl, adminEmail || 'admin', user?.Name, role, appDeepLink);
      console.log(
