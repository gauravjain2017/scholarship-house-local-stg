...
exports.updateProfile = async (req, res) => {
  try {
    ...
    let updated;
    try {
      // Persist the split names too, not just the combined Name. The mobile/web
      // forms read FirstName/LastName back on load; without storing them, an
      // edit would update Name but leave stale FirstName/LastName, so the form
      // would show the OLD first/last name after saving.
      const nameParts = finalName.split(/\s+/);
      const resolvedFirst = (firstName || '').trim() || nameParts[0] || '';
      const resolvedLast = (lastName || '').trim() || nameParts.slice(1).join(' ');
      ...
      const fieldUpdates = {
        ...
        Phone: resolvedPhone,
        // These records carry BOTH PascalCase and camelCase copies of the same
        // fields, and the readers (mobile `normalizeProfile`, admin) prefer the
        // camelCase ones. If we only update PascalCase, the stale camelCase
        // values shadow them on read — so keep both casings in sync.
        firstName: resolvedFirst,
        lastName: resolvedLast,
        phone: resolvedPhone,
      };
      if (address !== undefined) fieldUpdates.Address = address;
      if (acquisitionSpecialist !== undefined) {
        fieldUpdates.acquisitionSpecialist = acquisitionSpecialist;
      }
      // Profile avatar — stored as ProfileImage (PascalCase) in DynamoDB to
      // match the Submitter model's allowlist. Mobile uploads to S3 first and
      // sends the public URL here.
      if (profileImage !== undefined) {
        fieldUpdates.ProfileImage = profileImage;
      }
    ...
    res.json({
      ...
      user: {
        ...
        acquisitionSpecialist: updated?.acquisitionSpecialist || acquisitionSpecialist || null,
        address: updated?.Address || address || null,
        profileImage: updated?.ProfileImage || profileImage || null,
