...
function ContentTab({ fields, data, blockId, onUpdateData }) {
  const visibleFields = fields.filter((f) => {
    // For grid widget, hide cell 3-8 fields when layout is 1x2
    if (data.layout === '1x2' || !data.layout) {
      const match = f.key.match(/^cell(\d+)/);
      if (match && parseInt(match[1]) > 2) return false;
    }
...
const ManageSubmitterHome = () => {
  ...
  useEffect(() => {
    const loadLayout = async () => {
      try {
        const res = await api.get('/manage-homepages/user');
    ...
    <div className="h-screen bg-app flex flex-col overflow-hidden">
      ...
      <header className="flex items-center justify-between px-4 md:px-6 h-14 bg-white border-b border-gray-200 shrink-0 z-20">
        <div className="flex items-center gap-1 text-sm text-gray-500">
          ...
          <span className="mx-1">&rsaquo;</span><span className="text-gray-800 font-medium">Manage Submitter Home Page</span>
        </div>
...
export default ManageSubmitterHome;
