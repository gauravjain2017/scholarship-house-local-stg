...
const getPublishedDeals = async (req, res) => {
  try {
    ...
    let deals = await dealStorage.getDealsByStatusScan(['published', 'sold', 'pending']);
...
const getMySubmissions = async (req, res) => {
  try {
    ...
    // Fetch all deals and filter by submitterEmail === req.user.email
    const result = await dynamoDB.send(
      new ScanCommand({
        TableName: TABLES.PROPERTIES,
      })
    );
    const myDeals = (result.Items || []).filter(
      (deal) =>
...
const updateMyDeal = async (req, res) => {
  try {
    ...
    const ALLOWED_FIELDS = ['title', 'description', 'price', 'discountPrice', 'status', 'submitterRelationship', 'streetAddress', 'addressLine2', 'city', 'stateRegion', 'postalCode', 'category', 'turnkey', 'turnkeyFurnished', 'bedrooms', 'bathrooms', 'yearBuilt', 'squareFootage', 'expiry_date', 'isHOA', 'hoaMonthlyFee', 'financingType', 'emd', 'downPayment', 'expectedCloseDate', 'financialInfo', 'subjLoanBalance', 'subjInterestRate', 'subjLoanMaturity', 'subjMonthlyPrincipal', 'subjMonthlyInterest', 'subjMonthlyTaxesInsurance', 'sellerLoanAmount', 'sellerInterestRate', 'sellerLoanMaturity', 'sellerMonthlyPayment', 'totalMonthlyPayment', 'strZoning', 'strConfidence', 'strListingLink', 'strDataSheetsLink', 'vacationRentalMarkets', 'travelMotivations', 'guestDemandInsights', 'valueAddOpportunities', 'localContacts', 'interiorImages', 'exteriorImages', 'additionalImages', 'videos', 'specialTags', 'additionalInfo',
    ];
    const ALLOWED_STATUSES = new Set(['pending', 'draft']);
    ...
    if (updates.status !== undefined) {
      ...
      if (!ALLOWED_STATUSES.has(nextStatus)) {
        return res.status(400).json({
          error: 'Submitters may only set status to pending or draft.',
        });
      }
...
const getPublicDealById = async (req, res) => {
  try {
    ...
    if (deal.status !== 'published' && deal.status !== 'sold') {
      return res.status(404).json({ error: 'Deal not found' });
    }
    ...
    delete deal.streetAddress;
    delete deal.postalCode;
